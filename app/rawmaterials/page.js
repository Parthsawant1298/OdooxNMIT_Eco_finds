  ...
  useEffect(() => {
    const fetchRawMaterials = async () => {
      try {
        ...
        const uniqueCategories = [...new Set(data.rawMaterials?.map(material => material.category).filter(Boolean) || [])];
        ...
        data.rawMaterials?.forEach(material => {
          if (material.createdBy && material.createdBy._id && material.createdBy.username) {
            userMap.set(material.createdBy._id, {
              id: material.createdBy._id,
              name: material.createdBy.username
            });
          }
        });
...
const handleBuyNow = async (materialId) => {
  ...
  const filterRawMaterials = useCallback(() => {
    ...
    return rawMaterials.filter(material => {
      ...
      // user filter
      if (selectedUser !== 'all' && material.createdBy._id !== selectedUser) {
        return false;
      }
  ...
  const groupedMaterials = useCallback(() => {
    ...
    if (selectedUser !== 'all') {
      ...
      users.forEach(user => {
        const materialsFromuser = filteredRawMaterials.filter(m => m.createdBy._id === user.id);

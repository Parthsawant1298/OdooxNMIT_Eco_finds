...
export async function POST(request) {
  try {
    ...
    const user = await User.findById(userId).select('businessAddress');
    const userLocation = user?.businessAddress || '';

    // Get all available raw materials with user details
    ...
    .populate({
      path: 'createdBy',
      select: 'name businessAddress businessType'
    })
    .lean();

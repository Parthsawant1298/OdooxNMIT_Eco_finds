...
const ProductCategories = () => {
  ...
  useEffect(() => {
    const fetchCategoriesAndProducts = async () => {
      try {
        ...
        // If no products at all, set empty state gracefully
        ...
        // Get all products that have images, fallback to all products if none have images
        let productsWithImages = allProducts.filter(
          (product) => product.mainImage || (product.images && product.images.length > 0),
        )
        
        // If no products have images, use all products (they'll get placeholder images)
        ...
        // If no categories found, create a fallback "All Products" category
        if (mainCategories.length === 0) {
          mainCategories.push({
            id: 'all-products',
            title: 'All Products',
            viewAllText: 'See more',
            products: productsWithImages.slice(0, 4),
            loading: false
          })
        }
        ...
        // Set fallback data instead of error state to keep the component functional
        setDealsProducts([])
        setTenPercentDeals([])
        setCategoryRows([])
        setMaxDiscount(0)
        setIsLoading(false)
        // Only set error for network-related issues
  ...
  if (categoryRows.length === 0) {
    return (
      <div className="py-8 sm:py-12 md:py-16 bg-gray-50">
        <div className="container mx-auto px-2 sm:px-4">
          <div className="flex flex-col justify-center items-center min-h-[300px] sm:min-h-[400px] space-y-4">
            <div className="text-6xl">📦</div>
            <div className="text-center">
              <h3 className="text-lg font-semibold text-gray-700 mb-2">No Products Available</h3>
              <p className="text-gray-500 text-sm px-4">Products will appear here once they're added to the marketplace.</p>
            </div>
            <a 
    ...
    <div className="py-4 sm:py-6 md:py-8 bg-gray-50">
      <div className="container mx-auto px-2 sm:px-4">
            ...
            <div className="bg-white border border-gray-200 rounded-lg p-3 sm:p-4 h-full">
              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-3 sm:mb-4 gap-2">
                <h2 className="text-base sm:text-lg md:text-xl font-bold text-gray-900">
                  <span className="block sm:inline">Featured Items</span>
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-3 sm:p-4">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-3 sm:mb-4 gap-2">
            <h2 className="text-base sm:text-lg md:text-xl font-bold text-gray-900">
              <span className="block sm:inline">Great Value Finds</span>
              <span className="block sm:inline"> | </span>
          ...
          {tenPercentDeals.length === 0 ? (
            <div className="flex items-center justify-center min-h-[150px] sm:min-h-[200px]">
              <p className="text-gray-500 text-xs sm:text-sm text-center px-4">No 10% discount items available at the moment.</p>
            </div>
          ) : (
            <div className="relative">

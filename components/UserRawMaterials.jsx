...
import { Star, Share, Search, Filter, X, ChevronDown, Tag, Package, ArrowUpRight, Sliders, Grid, List, Percent, Edit, Trash, Plus, ChevronRight, User } from 'lucide-react';
import Header from '@/components/Header';
import Footer from '@/components/Footer';

export default function UserRawMaterials() {
  ...
  const handleDeleteRawMaterial = async () => {
    ...
    try {
      const response = await fetch(`/api/user/rawmaterials?id=${deleteRawMaterialId}`, {
        method: 'DELETE',
      });
  ...
  // Group materials by category and subcategory
  const groupedMaterials = useCallback(() => {
    const grouped = {};
    
    // Since this is user page, all materials are from the same user
    const userName = user?.sellerName || 'My Items';
    
    if (selectedCategory !== 'all') {
      // Group by subcategory when a specific category is selected
      grouped[userName] = {};
      grouped[userName][selectedCategory] = {};
      ...
      if (selectedSubcategory !== 'all') {
        ...
        if (materialsInSubcategory.length > 0) {
          grouped[userName][selectedCategory][selectedSubcategory] = materialsInSubcategory;
        }
      } else {
        ...
        subcategoriesInCategory.forEach(subcategory => {
          ...
          if (materialsInSubcategory.length > 0) {
            grouped[userName][selectedCategory][subcategory] = materialsInSubcategory;
          }
        });
      }
    } else {
      ...
      categories.forEach(category => {
        ...
        if (materialsInCategory.length > 0) {
          ...
          subcategoriesInCategory.forEach(subcategory => {
            ...
            if (materialsInSubcategory.length > 0) {
              grouped[userName][category][subcategory] = materialsInSubcategory;
            }
          });
        }
      });
    ...
    return grouped;
  }, [filteredRawMaterials, selectedCategory, selectedSubcategory, categories, user]);
  ...
  if (isLoading) {
    return (
      <div className="flex-grow flex items-center justify-center min-h-screen bg-gradient-to-br from-green-50 via-gray-50 to-white">
        <div className="flex flex-col items-center">
          <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-green-500 mb-4"></div>
          <p className="text-green-700 animate-pulse">Loading raw materials...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-50 via-gray-50 to-white">
      <Header />
      ...
      <div className="container mx-auto px-4 pb-16">
        ...
        <div className="py-8">
          <div className="flex justify-between items-center">
            <div>
              <h1 className="text-3xl font-bold text-gray-900 mb-2">My Raw Materials</h1>
              <p className="text-gray-600">Manage your raw materials and track their performance</p>
            </div>
            <Link 
              href="/add-raw-material" 
              className="flex items-center space-x-2 bg-green-600 hover:bg-green-700 text-white px-6 py-3 rounded-md transition-colors shadow-md"
            >
              <Plus size={18} />
            ...
            <div className="bg-white rounded-xl shadow-md p-5 sticky top-24">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-lg font-bold text-gray-800 flex items-center">
                  <Sliders size={18} className="mr-2 text-green-600" />
                  Filters
                </h2>
                {activeFilters > 0 && (
                  <button
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    <div 
                      className={`flex items-center px-3 py-2 rounded-md cursor-pointer ${selectedCategory === 'all' ? 'bg-green-50 text-green-700' : 'hover:bg-gray-50'}`}
                      onClick={() => {
                        setSelectedCategory('all');
                        setSelectedSubcategory('all');
                      }}
                    ...
                    {categories.map(category => (
                      <div key={category}>
                        <div 
                          className={`flex items-center px-3 py-2 rounded-md cursor-pointer ${selectedCategory === category ? 'bg-green-50 text-green-700' : 'hover:bg-gray-50'}`}
                          onClick={() => {
                        ...
                        {selectedCategory === category && subcategories[category] && subcategories[category].length > 0 && (
                          <div className="ml-6 mt-2 space-y-1">
                            <div
                              className={`flex items-center px-3 py-1 text-sm rounded-md cursor-pointer ${selectedSubcategory === 'all' ? 'bg-green-50 text-green-700' : 'hover:bg-gray-50'}`}
                            ...
                            {subcategories[category].map(subcategory => (
                              <div 
                                key={subcategory}
                                className={`flex items-center px-3 py-1 text-sm rounded-md cursor-pointer ${selectedSubcategory === subcategory ? 'bg-green-50 text-green-700' : 'hover:bg-gray-50'}`}
                  ...
                  <div className="px-2">
                    ...
                    <div className="flex items-center space-x-2">
                      <input
                        type="number"
                        placeholder="Min"
                        className="w-full pl-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500 text-sm"
            ...
            <div className="mb-6 bg-white p-4 rounded-xl shadow-md">
              <div className="flex flex-col md:flex-row gap-4">
                ...
                <button
                  ...
                  className="lg:hidden flex items-center justify-center space-x-2 bg-green-600 text-white py-2 px-4 rounded-md hover:bg-green-700 transition-colors"
                ...
                <div className="relative">
                  <select
                    className="appearance-none pl-4 pr-10 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500 bg-white"
                    value={sortBy}
              ...
              <div className="text-center py-12 bg-white rounded-xl shadow-md">
                ...
                {rawMaterials.length === 0 ? (
                  ...
                  <>
                    ...
                    <Link 
                      href="/add-raw-material"
                      className="inline-flex items-center px-6 py-3 bg-green-600 text-white rounded-md hover:bg-green-700 transition-colors shadow-md"
                    >
              ...
              <div className="space-y-10">
                {Object.entries(materialsByUser).map(([userName, categoryGroups]) => (
                  <div key={`user-${userName}`} className="bg-white rounded-xl shadow-md overflow-hidden">
                    {/* User header */}
                    <div className="bg-gradient-to-r from-green-500 to-green-700 p-4 text-white flex items-center justify-between">
                      <div className="flex items-center">
                        <User size={20} className="mr-3" />
                        <div>
                          <h2 className="text-xl font-bold">{user?.sellerName || 'My Items'}</h2>
                          <p className="text-green-100 text-sm">Added by you</p>
                        </div>
                      </div>
                      <div className="flex items-center space-x-2">
                        <span className="text-sm bg-white text-green-700 px-3 py-1 rounded-full">
                                      ...
                                      <div key={`${material._id}-${index}`} className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-shadow group max-w-md mx-auto">
                                        ...
                                        >
                                          ...
                                          <div className="absolute top-2 right-2 flex space-x-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                            <Link
                                              href={`/edit-raw-material/${material._id}`}
                                          ...
                                          <div className="flex flex-col space-y-3">
                                            ...
                                            <div className="flex space-x-2">
                                              <Link
                                                ...
                                                className="flex-1 bg-green-600 text-white py-2 px-3 rounded-md hover:bg-green-700 transition-colors flex items-center justify-center space-x-1"
                                              >
                                        ...
                                        <div className="flex-1 p-5 flex flex-col justify-between">
                                          ...
                                          <div className="flex items-center justify-between mt-4">
                                            ...
                                            <div className="flex space-x-2">
                                              <Link
                                                href={`/rawmaterials/${material._id}`}
                                                className="bg-green-600 text-white px-3 py-2 rounded-md hover:bg-green-700 transition-colors flex items-center space-x-1"
                                              >
                                                <span className="text-sm">View Details</span>
                                              </Link>
                                              <Link
